...
void init_controller_params(ControllerStruct *controller){
    ...
    controller->k_d = K_D;          //k_d=0.0001*Current loop bandwidth
    controller->k_q = K_Q;          //k_q=0.0001*Current loop bandwidth	
...
void commutate(ControllerStruct *controller, ObserverStruct *observer, GPIOStruct *gpio, float theta){
        ...
        if(fabs(controller->i_q) > 0.1f){
            observer->resistance = (controller->v_q - SQRT3*controller->dtheta_elec*(WB))/controller->i_q;
        }
        else{
            observer->resistance = R_PHASE;
        }
        if(isnan(observer->resistance) || observer->resistance < 0.0f || observer->resistance > 100.0f){observer->resistance = R_PHASE;}
        observer->temperature2 = (double)(25.0f + ((observer->resistance/R_PHASE)-1.0f)/0.00393f);
...
//新添加 2026年5月4日
	if(controller->p_des>=PI)
	{
		controller->p_des=PI;
	}
	else if(controller->p_des<=-PI)
	{
		controller->p_des=-PI;
	}
