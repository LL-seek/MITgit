...
void Init_SPI_CS2(void){
			// 使能GPIOD时钟
			RCC->AHB1ENR |= RCC_AHB1ENR_GPIODEN;

			// 配置PD2为输出模式
			GPIOD->MODER &= ~(GPIO_MODER_MODER2);  // 清除模式位
			GPIOD->MODER |= (1 << GPIO_MODER_MODER2_Pos);  // 输出模式(01)

			// 配置为低速输出
			GPIOD->OSPEEDR &= ~(GPIO_OSPEEDR_OSPEED2);  // 清除速度位，设置为低速(00)
			GPIOD->OSPEEDR |= (3 << GPIO_OSPEEDR_OSPEED2_Pos);  // 高速

			// 配置为推挽输出
			GPIOD->OTYPER &= ~(1 << 2);  // 推挽输出(0)

			// 可选：配置上拉/下拉电阻（根据实际需要）
			GPIOD->PUPDR &= ~(GPIO_PUPDR_PUPDR2);  // 无上拉下拉(00)
...
//			// 配置PD2为输出模式
//			GPIOD->MODER &= ~(GPIO_MODER_MODER2);  // 清除模式位
//			GPIOD->MODER |= (2 << GPIO_MODER_MODER2_Pos);  // 复用功能(10)
...
//			GPIOD->PUPDR &= ~(GPIO_PUPDR_PUPDR2);  // 无上拉下拉(00)	
//			
//			// 配置复用功能为AF6（通常SPI3_NSS为AF6）
//			GPIOD->AFR[0] &= ~(0xF << (4 * 2));  // 清除AFRL2
//			GPIOD->AFR[0] |= (6 << (4 * 2));     // AF7 = USART3_RTS			
		}	
