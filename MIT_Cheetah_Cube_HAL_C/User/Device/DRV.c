...
HAL_StatusTypeDef calibrate(void)                                    //执行DRV8323的CSA校准，返回HAL状态
{
    HAL_StatusTypeDef status;                                       //保存本次SPI通信的返回状态
    uint16_t val;                                                   //保存校准前的CSA寄存器配置

    status = read_register(CSACR, &val);                             //读取当前CSA配置，将寄存器值保存到val
    if (status != HAL_OK)                                            //读取未成功时结束本次校准
    {
        return status;                                               //将读取失败状态返回给调用方
    }

    status = write_register(CSACR, (uint16_t)(val | 0x001CU));        //将bit4、bit3、bit2置1，启动A、B、C三路CSA校准
    if (status != HAL_OK)                                             //校准命令写入未成功时结束本次校准
    {
        return status;                                                //将写入失败状态返回给调用方
    }

    HAL_Delay(1U);                                                    //等待至少1ms，使内部校准完成；仅在启动阶段调用

    return write_register(CSACR, (uint16_t)(val & ~0x001CU));         //清除三个校准位，保留其他配置，恢复正常采样并返回写入状态
}
...
if (status == HAL_OK)              //前一步返回HAL_OK时执行CSA校准
{
    status = calibrate();          //执行CSA校准，并保存返回的HAL状态
}
